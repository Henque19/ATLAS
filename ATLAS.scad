...
modelScale = 200; // Set to 1 for full scale, 200 fits Ultimaker 3D printer
defaultRes = 1; // Set to 1 for lower resolution, 0 for default

// Available parts:
//BeamPipe
//InnerDetector		"PRT"		Pixel Sct Trt
...
//		MirrDupCut()			BeamPipe();
//		MirrDupCut(0,30,-30)	InnerDetector("ST");
//		MirrDupCut(700,35,-35)	Calorimeters("L");
		MirrDupCut(1500,45,-45)	Calorimeters("R");
//		MirrDupCut(5400,60,-60)	MagnetSystem("B");
//		MirrDupCut(0)			MagnetSystem("E");
//		MirrDupCut(3000,50,-50)	MuonSpectrometer("I");
//		MirrDupCut(7000,80,-80)	MuonSpectrometer("O");
//		MirrDupCut(0,90,-90)	MuonSpectrometer("M");
//		MirrDupCut()			MuonSpectrometer("SLF");
//		MirrDupCut()			CERNFeet("B");
	}
...
module MirrDupCut(origin=-1,angle1=45, angle2=-45,length=40000){
	difference(){
		for (i = [0 : $children-1]){
			union(){
				child(i);
...
module BeamPipe(){
	rotate([0,90,0]) color("yellow") difference(){
...
module MagnetSystem(str){
	...
	if (search("B",str,1)!=[]) BarrelToroid(defaultRes);
...
module FCal(){
	rotate([0,90,0]) translate([0,0,3623+630+20+500+40]) color("gold") difference(){
		union(){
...
module Tile(str){
	if (search("T",str,1)!=[] || search("E",str,1)!=[]) difference(){
		rotate([0.5/64*360,0,0]) union(){
			if (search("T",str,1)!=[]){
				rotate([0,90,0]) color("gray") difference(){
					cylinder(h=2820, r=4250, $fn=64);
					cylinder(h=2820, r=2350, $fn=64);
				}
			}
			if (search("E",str,1)!=[]){
				rotate([0,90,0]) color("gray") difference(){
	...
	if (search("T",str,1)!=[] || search("R",str,1)!=[]){
		TileLongRail();
		mirror([0,1,0]) TileLongRail();
	}
	if (search("E",str,1)!=[] || search("R",str,1)!=[]){
		TileExtendedRail();
...
module EndCap(){
	translate([10130,0,0]) rotate([0,90,0]) difference(){
		linear_extrude(height = 5000, center = true, convexity = 10) import(file = "EndCap.dxf", layer="EndCap");
...
module BarrelToroid(defaultRes){
	strutPos = [1712.5,5137.5,8245.5,10845];
	for(i = [0:7]){
		rotate([22.5+45*i,0,0]) union(){
			BarrelToroidHalf(defaultRes);
			translate([0,0,9400/2+1100/2+5320-1100]) rotate([180,0,0]) translate([0,0,-9400/2-1100/2]) BarrelToroidHalf(defaultRes);
			for(j=[0:3]){
...
module BarrelToroidHalf(defaultRes){
	difference(){
		union(){
			translate([12630-1100/2,0,9400/2]) cylinder(h=5320/2, r=1100/2, $fn=defaultRes*12);
			translate([0,0,9400/2+1100/2]) rotate([0,90,0]) cylinder(h=12630, r=1100/2, $fn=defaultRes*12);
			translate([12630-1100/2-1100/4,0,9400/2+1100/2+1100/4]) rotate([0,45,0]) cylinder(h=4000, r=1100/2, center=true, $fn=defaultRes*12);
		}
		difference(){
		...
		difference(){
			...
			translate([12630-1100/2,0,9400/2]) cylinder(h=5320, r=1100/2, $fn=defaultRes*12);
		}
		difference(){
...
module BO(){
	for(i=[0:7]){
		rotate([45*i,0,0]) translate([12000/2,0,9477]){
			difference(){
		...
		rotate([22.5+45*i,0,0]) translate([12000/2,0,10350]){
			difference(){
